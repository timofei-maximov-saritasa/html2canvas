...
var parseTextBounds = function (context, value, styles, node) {
    var textList = breakText(value, styles, node);
    var textBounds = [];
    var offset = 0;
...
var breakText = function (value, styles, node) {
    return styles.letterSpacing !== 0 ? text_segmentation_1.splitGraphemes(value) : breakLines(value, styles, node);
};
var breakLines = function (value, styles, node) {
    var words = breakWords(value, styles);
    var lines = [];
    var offset = 0;
    var acc = '';
    for (var _i = 0, words_1 = words; _i < words_1.length; _i++) {
        var word = words_1[_i];
        var rects = createRange(node, offset, acc.length + word.length).getClientRects();
        if (rects.length <= 1) {
            acc += word;
        }
        else {
